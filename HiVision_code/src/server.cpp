...
class HiVisionPara {
	public:
		char *shpPath=NULL;
	// Path of the input shapefiles
	char *indexPath=NULL;
	// Path to store the spatial indexes
	char *patternPath=NULL;
	// Path of the input patterns
	char *redisHost=NULL;
	// Host IP of Redis
	int redisPort=6379;
	// Host Port of Redis
	int servicePort=10080;
	// Port of the HiVision service
}
;
HiVisionPara hvpara;
/** 
...
bool Linestring( char* shpFile, char* outIndex,char* redishost,char* rediskey,int redisport) {
	...
	long size =300000;
	// Max size (MB) of the spatial index mapped file
		...
		if(eType == wkbPolygon) {
			...
			for (int i=1;i<pointCount;i++) {
				...
				if (fabs(py1-py0)<tolerance) {
					...
					if(py0>py1)
											rtree_ptr->insert(boost::make_tuple(segment(point(px0-tolerance*(px1-px0)/(py1-py0), py0-tolerance),point(px1,py1)),polygonID,true)); else
			...
			for (int j=0;j<innerCount;j++) {
				...
				for (int i=1;i<pointCount;i++) {
					...
					maxy=maxy>py1? maxy:py1;
					if (fabs(py1-py0)<tolerance)
											rtree_ptr->insert(boost::make_tuple(segment(point(px0, py0),point(px1,py1)),polygonID,false)); else if(change*(py0-py1)<0) {
						if(py0>py1)
													rtree_ptr->insert(boost::make_tuple(segment(point(px0-tolerance*(px1-px0)/(py1-py0), py0-tolerance),point(px1,py1)),polygonID,true)); else
													rtree_ptr->insert(boost::make_tuple(segment(point(px0+tolerance*(px1-px0)/(py1-py0), py0+tolerance),point(px1,py1)),polygonID,true));
					} else
...
struct ServerMiddleware {
	...
	;
	void before_handle(crow::request& 
	/*req*/
	, crow::response& 
	/*res*/
	, context& 
		...
		try {
			...
			if(type==0)//type=0 Register point dataset {
				...
				if (Point(tmp1,tmp2,redisHost,tmp3,redisPort))
									res.write("[DONE] Register done!"); else 
									throw "Failed";
